...
function resetTimer() {
  clearInterval(timer);
  timer = setInterval(() => changeRight(), 500000);
}
...
function reset() {
  ...
  images[images.length - 2].style.transform = "translateX(-50%)";

  images[images.length - 1].style.opacity = "1";
  plaveSmrtovnice[images.length - 1].style.background = null;
  respectMax();
}
